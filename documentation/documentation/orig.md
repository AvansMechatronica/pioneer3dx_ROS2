...
p3dx is a ROS2 port of the [Pioneer 3DX](https://github.com/AvansMechatronica/pioneer3dx_ROS2) package. 

[based on](https://github.com/linorobot/linorobot2)

Once the robot's URDF has been configured in p3dx_description package, users can easily switch between booting up the physical robot and spawning the virtual robot in Gazebo. 

![p3dx_architecture](../images/p3dx_launchfiles.png)

Assuming you're using one of the tested sensors, p3dx automatically launches the necessary hardware drivers, with the topics being conveniently matched with the topics available in Gazebo. This allows users to define parameters for high level applications (ie. Nav2 SlamToolbox, AMCL) that are common to both virtual and physical robots.

The image below summarizes the topics available after running **bringup.launch.py**.
![p3dx_microcontroller](../images/microcontroller_architecture.png)
...
#### 2.1 Install p3dx Package
Install p3dx package on the host machine:
...
* microxrcedds_agent and micro_ros_agent dependency checks are skipped to prevent this [issue](https://github.com/micro-ROS/micro_ros_setup/issues/138) of finding its keys. This means that you have to always add `--skip-keys microxrcedds_agent --skip-keys micro_ros_agent` whenever you have to run `rosdep install` on the ROS2 workspace where you installed p3dx. 
...
All the hardware documentation and robot microcontroller's firmware can be found [here](https://github.com/Pioneer 3DX/p3dx_hardware).

## URDF
### 1. Define robot properties
[p3dx_description](./p3dx_description) package has parameterized xacro files that can help you kickstart writing the robot's URDF. Open <robot_type>.properties.urdf.xacro in [p3dx_description/urdf](./p3dx_description/urdf) directory and change the values according to the robot's specification/dimensions. All pose definitions must be measured from the `base_link` (center of base) and wheel positions (ie `wheel_pos_x`) are referring to wheel 1.

For custom URDFs, you can change the `urdf_path` in [description.launch.py](./p3dx_description/launch/description.launch.py) found in p3dx_description/launch directory. 
...
#### 2.1 Publish the URDF from the robot computer:
    ...
    ros2 launch p3dx_description description.launch.py
...
ros2 launch p3dx_description description.launch.py rviz:=true
```
...
```bash
ros2 launch p3dx_viz robot_model.launch.py
```
## Quickstart
...
```bash
ros2 launch p3dx_bringup bringup.launch.py base_serial_port:=/dev/ttyACM1
```

- **micro_ros_baudrate** - micro-ROS serial baudrate. default 115200.

```bash
ros2 launch p3dx_bringup bringup.launch.py base_serial_port:=/dev/ttyUSB0 micro_ros_baudrate:=921600
```
...
ros2 launch p3dx_bringup bringup.launch.py micro_ros_transport:=udp4 micro_ros_port:=8888
```
...
ros2 launch p3dx_bringup bringup.launch.py madgwick:=true orientation_stddev:=0.01

p3dx_ws/src/p3dx/p3dx_base/config/ekf.yaml
    imu0: imu/data
...
```
p3dx_bringup.launch.py or gazebo.launch.py must always be run on a separate terminal before creating a map or robot navigation when working on a real robot or gazebo simulation respectively.
...
Pass `joy` argument to the launch file and set it to true to enable the joystick. For example:
    ...
    ros2 launch p3dx_bringup bringup.launch.py joy:=true
...
For example:
```bash
ros2 launch p3dx_navigation slam.launch.py rviz:=true sim:=true
```
...
```bash
ros2 launch p3dx_viz slam.launch.py
```
...
Open p3dx/p3dx_navigation/launch/navigation.launch.py and change *MAP_NAME* to the name of the newly created map. Build the robot computer's workspace once done:
...
```bash
ros2 launch p3dx_navigation navigation.launch.py map:=<path_to_map_file>/<map_name>.yaml
```
...
The `rviz` argument for navigation.launch.py won't work on headless setup but you can visualize the robot remotely from the host machine:
    ...
    ros2 launch p3dx_viz navigation.launch.py
...
- Try to up `transform_timeout` by 0.1 in p3dx_navigation/config/slam.yaml until the warning is gone.
