...
void initFS() {
  ...
  if (!result) {
    DEBUG_PRINT("An error has occurred while mounting spif_fs\n");
#ifndef IGNOR_TFT_PRINT
    tft_printf(ST77XX_BLUE, "No filesystem\ndetected\nInstall by\nPlatfomIO\n");
#endif
    while(true){};
  }
  DEBUG_PRINT("spif_fs mounted successfully\n");
}


// Read File from spif_fs
...
bool configureNetwork(bool forceConfigure, NETWORK_CONFIG *networkConfig) {
  ...
  // Load values saved in spif_fs
  ssid = readFile(SPIFFS, ssidPath);
  pass = readFile(SPIFFS, passPath);
  ros_agent_ipPath = readFile(SPIFFS, ros_server_ipPath);
  ros_agent_port = readFile(SPIFFS, ros_server_portPath);
      ...
      for(int i=0;i<params;i++){
        ...
        if(p->isPost()){
          ...
          if (p->name() == PARAM_INPUT_3) {
            ...
            // Write file to save value
            writeFile(SPIFFS, ros_server_ipPath, ros_agent_ipPath.c_str());
          }
          ...
          if (p->name() == PARAM_INPUT_4) {
            ...
            // Write file to save value
            writeFile(SPIFFS, ros_server_portPath, ros_agent_port.c_str());
          }
        }
      }
