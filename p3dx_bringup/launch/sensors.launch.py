...
def generate_launch_description():
    laser_sensor_name = os.getenv('p3dx_LASER_SENSOR', '')
    depth_sensor_name = os.getenv('p3dx_DEPTH_SENSOR', '')
    
    fake_laser_config_path = PathJoinSubstitution(
        [FindPackageShare('p3dx_bringup'), 'config', 'fake_laser.yaml']
    )
    ...
    laser_launch_path = PathJoinSubstitution(
        [FindPackageShare('p3dx_bringup'), 'launch', 'lasers.launch.py']
    )

    depth_launch_path = PathJoinSubstitution(
        [FindPackageShare('p3dx_bringup'), 'launch', 'depth.launch.py']
    )
